    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          TelmedNavBar(navOpacity: _navOpacity), // Using the common widget
        ],
      ),
    );
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          const _FaqSection(),
          const TelmedFooter(), // Using the common widget
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          const _SectionTitle(
            ...
            subtitle: "Find detailed answers about our KSH 54 call sessions and homecare services.",
          ),
            ...
            children: [
              Expanded(
                child: Column(
                  children: const [
                    _FaqTile(
                      "When exactly do I pay the KSH 54 fee?",
                      "The KSH 54 fee is paid upfront before the phone call begins. Once you tap 'Call Telmed', you will be prompted to complete the payment via M-Pesa. As soon as the transaction is confirmed, you are instantly connected to the next available doctor for your consultation.",
                    ),
                    _FaqTile(
                      "How long does a KSH 54 voice call consultation last?",
                      "We do not set strict time limits. The KSH 54 fee covers the complete session required for the doctor to understand your symptoms, provide a diagnosis, and issue a prescription.",
                    ),
                    _FaqTile(
                      "What medical conditions can Telmed doctors treat over a call?",
                      "Our doctors can treat conditions such as malaria symptoms, UTIs, respiratory infections, stomach issues, and skin rashes. If the doctor determines your case is a physical emergency, they will advise you on the nearest clinic.",
                    ),
                    _FaqTile(
                      "Is the KSH 54 fee refundable if the doctor can't help me?",
                      "If our doctor determines within the first two minutes that your condition requires an immediate physical visit to an ER, we credit that KSH 54 back to your Telmed account.",
                    ),
                    _FaqTile(
                      "Can I call a doctor for my child using this service?",
                      "Absolutely. Many parents use the KSH 54 call service for pediatric advice on fevers, coughs, or rashes. Our doctors are trained to guide parents through basic checks.",
                    ),
                  ],
                ),
              ),
              ...
              Expanded(
                child: Column(
                  children: const [
                    _FaqTile(
                      "Are the doctors on these calls fully qualified?",
                      "Yes. Every doctor answering a Telmed call is a licensed medical professional registered with the KMPDC. We do not use 'bots' for our consultations.",
                    ),
                    _FaqTile(
                      "How do I get my prescription after the call?",
                      "Immediately following the voice session, the doctor generates a digital prescription. This is sent to your Telmed App and shared with you via WhatsApp or SMS.",
                    ),
                    _FaqTile(
                      "Does the call fee include the lab tests?",
                      "No, the KSH 54 fee covers the professional medical advice and prescription only. Lab tests and home nursing are billed separately based on the specific procedure.",
                    ),
                    _FaqTile(
                      "What if I need to talk to the same doctor again?",
                      "Follow-up calls within 24 hours regarding the same condition are prioritized. All our doctors have access to your digital file to ensure continuity of care.",
                    ),
                  ],
                ),
              ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text("Call to Clinic Promise", style: GoogleFonts.plusJakartaSans(fontSize: 40, fontWeight: FontWeight.bold, color: Colors.white)),
          const SizedBox(height: 25),
          SizedBox(
            width: 800,
            child: Text(
              ...
              style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 20, height: 1.6, fontStyle: FontStyle.italic),
            ),
          ),
        ],
      ),
...
class _ModernStepProcess extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const Padding(
      padding: EdgeInsets.symmetric(vertical: 120, horizontal: 100),
      child: Column(
        children: [
          _SectionTitle(label: "HOW IT WORKS", title: "Minutes to Wellness", subtitle: "Simple steps to get the assistance you need."),
          SizedBox(height: 80),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceAround,
            children: [
        ...
        children: [
          Expanded(
            ...
            child: Container(
              ...
              decoration: const BoxDecoration(color: Color(0xFFF0F4F2), borderRadius: BorderRadius.only(topLeft: Radius.circular(40), bottomLeft: Radius.circular(40))),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  ...
                  const SizedBox(height: 20),
                  Text("Voice Session\nfor KSH 54.", style: GoogleFonts.plusJakartaSans(fontSize: 44, fontWeight: FontWeight.w800, color: const Color(0xFF1B4D2C))),
                  const SizedBox(height: 30),
          ...
          Expanded(
            ...
            child: Container(
              decoration: const BoxDecoration(
                gradient: LinearGradient(colors: [Color(0xFF2D7D46), Color(0xFF4CAF50)]),
...
class _TestimonialWall extends StatelessWidget {
  const _TestimonialWall();
  @override
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Column(
        children: [
          _SectionTitle(label: "TESTIMONIALS", title: "Loved by Millions", subtitle: "Real stories from our patients."),
          SizedBox(height: 60),
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                _ReviewCard("Mercy W.", "I called at 2 AM for my son's fever. The KSH 54 call saved us!"),
                _ReviewCard("David O.", "Paid my 54 bob, got connected in 30 seconds. Best healthcare app."),
                _ReviewCard("Sarah K.", "Prescription arrived 1 minute after the call ended."),
              ],
            ),
          ),
        ],
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Text(title, textAlign: TextAlign.center, style: GoogleFonts.plusJakartaSans(fontSize: 48, fontWeight: FontWeight.w800)),
        const SizedBox(height: 15),
        SizedBox(width: 600, child: Text(subtitle, textAlign: TextAlign.center, style: const TextStyle(fontSize: 18, color: Colors.black45))),
      ],
    );
  }
}
...
class _ReviewCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(width: 350, margin: const EdgeInsets.only(right: 30), padding: const EdgeInsets.all(40), decoration: BoxDecoration(color: const Color(0xFFFAFAFA), borderRadius: BorderRadius.circular(25)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.star, color: Color(0xFFF9A825)), const SizedBox(height: 20), Text('"$text"', style: const TextStyle(fontStyle: FontStyle.italic)), const SizedBox(height: 20), Text(name, style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF2D7D46)))]));
  }
}
